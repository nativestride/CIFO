...
from solution import (
    InsufficientPlayersForPositionError,
...
def hill_climbing(
    ...
    **kwargs,
    ...
    if not hasattr(initial_solution, "get_neighbors"):
        raise TypeError(
            "initial_solution for hill_climbing must have a 'get_neighbors' method."
        )
    ...
    for iteration in range(max_iterations):
        neighbors = (
            current_sol.get_neighbors()
        )  # This method should return new solution objects

        if not neighbors:
            if verbose:
                print(
                    f"Hill Climbing: No valid neighbors found at iteration {iteration}. Stopping."
                )
            break
        ...
        best_neighbor_fitness = (
            best_neighbor.fitness()
        )  # Could cache this if fitness is expensive
        ...
        if best_neighbor_fitness < current_fitness:
            ...
            if verbose:
                print(
                    f"Hill Climbing: Iteration {iteration}, Fitness = {current_fitness:.6f}"
                )
        else:
            ...
            if iterations_without_improvement >= max_no_improvement:
                if verbose:
                    print(
                        f"Hill Climbing: Stopping after {max_no_improvement} iterations without improvement (Iter {iteration})."
                    )
                break
...
def simulated_annealing(
    ...
    verbose=False,
    safe_exp_func=safe_exp,  # Parameter for the safe exponential function
    **kwargs,  # For other potential params through config
    ...
    if not hasattr(initial_solution, "get_random_neighbor"):
        raise TypeError(
            "initial_solution for simulated_annealing must have a 'get_random_neighbor' method."
        )
    ...
    best_solution = deepcopy(
        current_solution
    )  # Deepcopy to store the best state independently
    ...
    if verbose:
        print(
            f"SA Initial: Temp={temperature:.2f}, Fitness={current_fitness:.6f}, Best Fitness={best_fitness:.6f}"
        )

    while temperature > min_temperature:
        for i_iter in range(iterations_per_temp):
            neighbor_solution = (
                current_solution.get_random_neighbor()
            )  # This returns a NEW solution instance
            neighbor_fitness = neighbor_solution.fitness()

            delta_fitness = neighbor_fitness - current_fitness

            accepted_move = False
            if (maximization and delta_fitness > 0) or (
                not maximization and delta_fitness < 0
            ):
            ...
            if accepted_move:
                current_solution = (
                    neighbor_solution  # Assign new instance, NO deepcopy needed here
                )
                ...
                if (maximization and current_fitness > best_fitness) or (
                    ...
                    best_solution = deepcopy(
                        current_solution
                    )  # Deepcopy for independent best record
                    best_fitness = current_fitness
                    if (
                        verbose and i_iter % 10 == 0
                    ):  # Print less frequently for new best
                        print(
                            f"  SA New Best: Temp={temperature:.2f}, Iter={i_iter}, Best Fitness={best_fitness:.6f}"
                        )
        ...
        if verbose:
            print(
                f"SA Cooled: Temp={temperature:.2f}, Current Fitness={current_fitness:.6f}, Best Fitness={best_fitness:.6f}"
            )
...
def hill_climbing_random_restart(
    ...
    tuple
):  # Returns: best_solution_overall, best_fitness_overall, history_of_best_fitness_per_restart
    """
    ...
    logger.info(
        f"Starting Hill Climbing with Random Restart: {num_restarts} restarts planned."
    )
    ...
    history_of_best_fitness_per_restart = (
        []
    )  # Tracks the best fitness achieved in each restart
    ...
    for i_restart in range(num_restarts):
        ...
        for attempt in range(MAX_INIT_ATTEMPTS):
            try:
                ...
                if temp_sol.is_valid():
                    ...
                    logger.info(
                        f"HCRR Restart {i_restart + 1}: Valid initial solution created on attempt {attempt + 1} (Fitness: {current_initial_sol.fitness():.4f})."
                    )
                    break
                else:
                    logger.debug(
                        f"HCRR Restart {i_restart + 1}, Attempt {attempt + 1}: Initial solution invalid."
                    )
            except InsufficientPlayersForPositionError as e_pos:
                logger.warning(
                    f"HCRR Restart {i_restart + 1}, Attempt {attempt + 1}: Failed to create initial solution due to position constraints - {e_pos}"
                )
            except Exception as e_init:
                logger.error(
                    f"HCRR Restart {i_restart + 1}, Attempt {attempt + 1}: Unexpected error during initial solution creation - {e_init}",
                    exc_info=True,
                )
        if current_initial_sol is None:
            logger.warning(
                f"HCRR Restart {i_restart + 1}: Failed to create a valid initial solution after {MAX_INIT_ATTEMPTS} attempts. Skipping this restart."
            )
            history_of_best_fitness_per_restart.append(
                float("inf")
            )  # Record failure for this restart
        ...
        hc_run_params = (
            hc_specific_kwargs.copy()
        )  # Start with a copy of hc_specific_kwargs
        ...
        logger.info(
            f"HCRR Restart {i_restart + 1} completed. Best fitness for this restart: {best_fit_this_restart:.4f}"
        )
        ...
        if best_fit_this_restart < best_fitness_overall:
            ...
            logger.info(
                f"HCRR Restart {i_restart + 1}: New GLOBAL best solution found! Fitness: {best_fitness_overall:.4f}"
            )

    if best_solution_overall is None and num_restarts > 0:
        logger.warning(
            "HCRR completed, but no valid solution was found that could be optimized across all restarts."
        )

    logger.info(
        f"Hill Climbing with Random Restart finished. Overall best fitness: {best_fitness_overall:.4f}"
    )
    return (
