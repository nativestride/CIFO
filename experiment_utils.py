...
# The actual solution classes for recreation will be looked up via the passed module.
...
class FitnessCounter:
    ...
    def start_counting(self, solution_class_to_wrap=LeagueSolution):
        logger.debug(f"Attempting to start fitness counting for: {solution_class_to_wrap.__name__}")
        if self._wrapped_class_ref != solution_class_to_wrap or self._original_fitness_method is None:
            if self._wrapped_class_ref and self._original_fitness_method:
                 logger.debug(f"Restoring original fitness to {self._wrapped_class_ref.__name__}")
            ...
            logger.debug(f"Original fitness method of {solution_class_to_wrap.__name__} stored and wrapped.")
            
            counter_instance = self
            def counting_wrapper_for_fitness(solution_object_instance, *args, **kwargs):
                counter_instance.count += 1
                return counter_instance._original_fitness_method(solution_object_instance, *args, **kwargs)
    ...
    def stop_counting(self) -> int:
        ...
        if self._original_fitness_method and self._wrapped_class_ref:
            self._wrapped_class_ref.fitness = self._original_fitness_method
            logger.debug(f"Original fitness method restored to {self._wrapped_class_ref.__name__}.")
            self._original_fitness_method = None 
            self._wrapped_class_ref = None
        
        current_eval_count = self.count
        self.count = 0
        logger.debug(f"Fitness counting stopped. Evaluations: {current_eval_count}. Count reset.")
        return current_eval_count

# --- Solution Data Serialization/Deserialization ---

def extract_solution_data_for_storage(solution_object_to_store: LeagueSolution) -> dict or None:
    ...
    if hasattr(solution_object_to_store, 'fitness'):
        try: _fitness_val = solution_object_to_store.fitness()
        except Exception as e: logger.warning(f"Fitness retrieval error during extraction: {e}")

    data = {
        'repr': list(solution_object_to_store.repr) if hasattr(solution_object_to_store, 'repr') else None,
    ...
    logger.debug(f"Extracted data for {data['solution_class_name']}: Fitness {data['fitness_value']:.4f}")
    return data

def recreate_solution_from_data(stored_solution_data: dict,
                                master_players_list_for_recreation: list,
                                problem_definition_for_recreation: dict,
                                solution_classes_module=None) -> 'LeagueSolution or None': # Use quotes for forward ref if LeagueSolution not fully defined here
    """
    ...
    sol_class_name_str = stored_solution_data.get('solution_class_name', 'LeagueSolution')
    logger.debug(f"Attempting to recreate solution of type: {sol_class_name_str}")
    
    ResolvedSolutionClass = None # Initialize to None

    if solution_classes_module:
        ResolvedSolutionClass = getattr(solution_classes_module, sol_class_name_str, None)
        if ResolvedSolutionClass:
            logger.debug(f"Found class '{sol_class_name_str}' in provided module.")
    
    if ResolvedSolutionClass is None: # Fallback if not found in the passed module or module not provided
        logger.debug(f"Class '{sol_class_name_str}' not found in provided module (or module was None). Checking globals() of experiment_utils.")
        ResolvedSolutionClass = globals().get(sol_class_name_str) # Try current module's (experiment_utils) globals
        if ResolvedSolutionClass:
            logger.debug(f"Found class '{sol_class_name_str}' in experiment_utils.globals().")


    if ResolvedSolutionClass is None:
        logger.error(f"Solution class '{sol_class_name_str}' could not be resolved. "
                       "Ensure it's defined, imported into 'solution.py' (if passing that module), "
                       "or available in the scope where this utility is called if no module is passed. Cannot recreate.")
        return None
    ...
    try:
        ...
        recreated_sol_object = ResolvedSolutionClass(
            ...
            num_teams=stored_solution_data.get('num_teams', problem_definition_for_recreation['num_teams']),
            team_size=stored_solution_data.get('team_size', problem_definition_for_recreation['team_size']),
            max_budget=problem_definition_for_recreation['max_budget'],
            position_requirements=problem_definition_for_recreation['position_requirements']
        ...
        if stored_solution_data.get('repr') is not None:
            ...
            logger.warning(f"Recreating solution for '{sol_class_name_str}' without a specific representation. "
                           "It will have a random initial representation from its class. Fitness may not align.")
        
        logger.debug(f"Successfully recreated solution object of type {sol_class_name_str}.")
        return recreated_sol_object
    except Exception as e_recreate:
        logger.error(f"Error during instantiation of solution object of type '{sol_class_name_str}': {e_recreate}")
        import traceback
...
def plot_summary_statistics_bars(final_results_df: pd.DataFrame):
    ...
    for metric_col in metrics:
        ...
        summary_stats = valid_data.groupby('Configuration')[metric_col].agg(['mean', 'std', 'min', 'max'])
        logger.info(f"\n📊 Statistics for: {metric_col}")
        
        fmt = "{:.4f}" if metric_col in ['BestFitness', 'RuntimeSeconds'] else "{:.1f}"
        formatters = {col: (lambda x, f=fmt: f.format(x)) for col in summary_stats.columns}
        try:
        ...
        plt.figure() 
        ax = summary_stats['mean'].plot(kind='bar', yerr=summary_stats['std'], capsize=4,
                                      color=sns.color_palette("viridis", len(summary_stats)), 
                                      edgecolor='grey')
        title_suffix = "(Lower is Better)"
        if metric_col == 'Iterations': title_suffix = "(Context Dependent)"
        ...
        for i, val_mean in enumerate(summary_stats['mean']):
            if np.isfinite(val_mean):
                ...
                offset_y = (std_err * 0.1 if pd.notna(std_err) else 0) + (val_mean * 0.02)
                ax.text(i, val_mean + offset_y, txt_val, ha='center', va='bottom', fontsize=8)
        plt.tight_layout()
        plt.show()
...
def plot_metric_distributions_boxplots(final_results_df: pd.DataFrame):
    ...
    for metric_col in metrics:
        if metric_col not in final_results_df.columns: continue
        valid_data = final_results_df[
        ...
        if not valid_data.empty:
            plt.figure()
            sns.boxplot(x='Configuration', y=metric_col, data=valid_data, 
                        palette="pastel", hue='Configuration', legend=False)
            plt.title(f'Distribution of {metric_col} by Algorithm', fontsize=14)
            plt.ylabel(metric_col, fontsize=11); plt.xlabel('Configuration', fontsize=11)
            plt.xticks(rotation=45, ha="right"); plt.grid(axis='y', linestyle=':', alpha=0.6)
            plt.tight_layout(); plt.show()
        else: logger.info(f"No valid data for box plot of '{metric_col}'.")

def plot_convergence_curves(final_history_map: dict):
    if not final_history_map: return
    logger.info("\n--- Convergence Plots ---")
    valid_configs = {k:v for k,v in final_history_map.items() if v}
    num_configs = len(valid_configs)
    if num_configs == 0: logger.info("No history data for convergence plots."); return

    rows = (num_configs + 1) // 2; cols = 2
    fig, axes = plt.subplots(rows, cols, figsize=(8*cols, 5*rows), squeeze=False)
    ax_flat = axes.flatten(); plot_idx = 0
    for config_name, runs_data in valid_configs.items():
        if plot_idx >= len(ax_flat): break
        ax = ax_flat[plot_idx]; has_data = False; num_labeled = 0
        for run_id, hist_list in runs_data.items():
            if hist_list:
                ...
                has_data = True; 
                if lbl: num_labeled += 1
        if has_data:
            ax.set_title(f'Convergence: {config_name}', fontsize=12)
            ax.set_xlabel('Iteration/Generation', fontsize=10); ax.set_ylabel('Best Fitness', fontsize=10)
            ax.grid(True, ls=':', alpha=0.5); 
            if num_labeled > 0: ax.legend(fontsize=8)
        else:
...
def rank_algorithms_custom(results_df: pd.DataFrame) -> pd.DataFrame or None:
    if results_df.empty: logger.warning("Ranking: Input DataFrame empty."); return None
    logger.info("\n--- Algorithm Performance Ranking ---")
    
    valid_df = results_df[ # Ensure we only use valid, finite results for ranking
        results_df['BestFitness'].notna() & np.isfinite(results_df['BestFitness']) &
        results_df['RuntimeSeconds'].notna() & np.isfinite(results_df['RuntimeSeconds']) &
        results_df['FunctionEvaluations'].notna() & np.isfinite(results_df['FunctionEvaluations']) &
        (~results_df['AlgorithmName'].str.contains("ERROR", na=False))
    ]
    if valid_df.empty: logger.warning("Ranking: No valid data after filtering."); return None
    ...
    stds = valid_df.groupby('Configuration')['BestFitness'].std().rename('StdBestFitness').fillna(0.0)
    ...
    ranks['ConsistencyRank'] = metrics['ConsistencyScore'].rank(method='min', ascending=False)
    ...
    display_cols = ['OverallRank'] + rank_cols + ['MeanBestFitness', 'StdBestFitness', 'MeanRuntime', 'MeanEvals', 'ConsistencyScore']
    ...
    fmts.update({'MeanBestFitness':"{:.4f}", 'StdBestFitness':"{:.4f}", 'MeanRuntime':"{:.3f}", 
                 'MeanEvals':"{:.1f}", 'ConsistencyScore':"{:.2f}"})
    ...
    if 'OverallRank' in final_ranks.columns:
        ...
        final_ranks['OverallRank'].sort_values().plot(kind='bar', color=sns.color_palette("YlOrRd_r", len(final_ranks)))
        plt.title('Overall Algorithm Ranking', fontsize=14); plt.ylabel('Overall Rank', fontsize=11)
        plt.xlabel('Configuration', fontsize=11); plt.xticks(rotation=45, ha="right")
        plt.tight_layout(); plt.show()
    return final_ranks
...
# Assuming recreate_solution_from_data is also in this experiment_utils.py
# and LeagueSolution might be imported for type hinting or FitnessCounter default.
# from .solution import LeagueSolution # Or just use forward reference string 'LeagueSolution'
...
def display_single_best_solution_details(
    ...
    if not best_sol_data:
        logger.info("No best solution data provided to display_single_best_solution_details.")
        return
    ...
    if not best_obj:
        logger.error("Could not recreate the best solution object for detailed display. Aborting details.")
        return
    ...
    recalculated_fitness_val = best_obj.fitness() # This call uses the original .fitness()
    logger.info(f"Recreated Best Solution: Is Valid = {is_recreated_valid}, Recalculated Fitness = {recalculated_fitness_val:.4f}")
    if not is_recreated_valid:
        logger.warning("The recreated overall best solution is flagged as INVALID. Displayed statistics might be misleading or incomplete.")

    # Get detailed team statistics from the solution object
    team_stats_list = best_obj.get_team_stats()
    if not team_stats_list:
        logger.warning("No team statistics could be generated for the overall best solution.")
        return
    ...
    for team_stat_item in team_stats_list:
        ...
        print(f"  Players ({team_stat_item.get('num_players',0)}): {team_stat_item.get('players_names', [])}")
        print(f"  Avg Skill: {team_stat_item.get('avg_skill', 0.0):.2f}")
        print(f"  Total Salary: {team_stat_item.get('total_salary', 0.0):.2f} (Budget Limit: {problem_def.get('max_budget', 'N/A')})")
        print(f"  Actual Positions: {team_stat_item.get('positions', {})} (Required: {problem_def.get('position_requirements', {})})")
        
        # Inline validity checks for quick visual reference during display
        if team_stat_item.get('total_salary', 0.0) > problem_def.get('max_budget', float('inf')):
            print("    🔴 WARNING: TEAM BUDGET EXCEEDED!")
        if team_stat_item.get('positions', {}) != problem_def.get('position_requirements', {}):
            print("    🔴 WARNING: TEAM POSITIONAL MISMATCH!")
        if team_stat_item.get('num_players',0) != problem_def.get('team_size',-1): # Compare with problem_def team_size
             print(f"    🔴 WARNING: TEAM SIZE IS {team_stat_item.get('num_players',0)}, "
    ...
    if team_stats_df_best_plot.empty:
        logger.warning("Team statistics DataFrame for the best solution is empty. Skipping detailed visualizations.")
        return
    ...
    if 'team_id' in team_stats_df_best_plot.columns:
        team_stats_df_best_plot['team_id'] = pd.to_numeric(team_stats_df_best_plot['team_id'], errors='coerce').fillna(-1) # Coerce and fill to handle issues
    ...
    if required_balance_cols.issubset(team_stats_df_best_plot.columns):
        fig_balance_best, (ax_skill_plot, ax_salary_plot) = plt.subplots(1, 2, figsize=(16, 5.5)) # Adjusted size
        
        # Average Skill by Team Plot
        sns.barplot(x='team_id', y='avg_skill', data=team_stats_df_best_plot, ax=ax_skill_plot, 
                    palette="coolwarm_r", hue='team_id', legend=False) # Added hue and legend=False
        ...
        sns.barplot(x='team_id', y='total_salary', data=team_stats_df_best_plot, ax=ax_salary_plot, 
                    palette="viridis_r", hue='team_id', legend=False) # Added hue and legend=False
        # Use label for axhline for the legend
        ax_salary_plot.axhline(problem_def.get('max_budget', float('inf')), 
                               color='red', linestyle='--', 
        ...
        fig_balance_best.suptitle(f'Team Balance Analysis for Best Solution ({config_name} - Fitness: {fitness:.4f})', fontsize=16)
        fig_balance_best.tight_layout(rect=[0, 0.03, 1, 0.94]) # Adjust rect for suptitle
        plt.show()
    else:
        logger.warning("Missing required columns ('team_id', 'avg_skill', 'total_salary') "
    ...
    if 'positions' in team_stats_df_best_plot.columns and not team_stats_df_best_plot['positions'].empty:
        try:
            # Expand the 'positions' dictionary column (which contains dicts) into separate columns
            positions_as_df_expanded = pd.json_normalize(team_stats_df_best_plot['positions'])
            
            # Ensure 'team_id' column exists for merging/melting.
            if 'team_id' not in team_stats_df_best_plot.columns:
                 logger.error("CRITICAL for position plot: 'team_id' column is missing in team_stats_df_best_plot.")
            ...
            actual_position_columns_present = [
                col for col in position_keys_expected if col in df_ready_for_position_melt.columns
            ]
            
            if not actual_position_columns_present:
                 logger.warning(f"No required position columns ({position_keys_expected}) found in DataFrame "
                ...
                if len(actual_position_columns_present) < len(position_keys_expected):
                    logger.warning(f"Only found columns {actual_position_columns_present} for position plot, "
                ...
                sns.barplot(x='team_id', y='PlayerCount', hue='PositionType', 
                            data=melted_position_data_best, palette="Set2")
                plt.title(f'Position Distribution by Team ({config_name} - Best Solution)', fontsize=14)
             ...
             logger.error(f"KeyError during position distribution plot generation (melt step): {e_key_error_melt}. "
                          "This might be due to 'team_id' or expected position columns being missing after normalization.")
        except Exception as e_position_plot_general: # Catch any other unexpected errors
            logger.error(f"An unexpected error occurred during position distribution plot generation: {e_position_plot_general}",
                         exc_info=True) # Log with traceback for more details
    else:
        logger.warning("Could not generate position distribution plot: 'positions' data column missing or empty in team_stats_df_best_plot.")
    
    logger.info(f"--- End of Detailed Analysis for Overall Best Solution ({config_name}) ---")