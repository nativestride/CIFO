...
class LeagueSolution(Solution):
    ...
    def __init__(
        ...
        if players is None:
            logger.warning(
                "LeagueSolution initialized without player data. Some operations might fail."
            )
            self.players = []
        else:
            self.players = (
                players  # Assumed to be a reference to a static list of player dicts
            )
        ...
        if position_requirements is None:
            if self.team_size == 7:  # Default for the common case
                ...
                logger.debug(
                    f"Using default position requirements for team_size=7: {self.position_requirements}"
                )
            else:
                raise ValueError(
    ...
    def __deepcopy__(self, memo):
        ...
        result._fitness_cache = deepcopy(
            self._fitness_cache, memo
        )  # If fitness caching is implemented
    ...
    def random_initial_representation(self) -> list:
        ...
        if not self.players:
            ...
            raise ValueError(
                "Cannot generate random representation: 'self.players' is empty."
            )
        ...
        player_to_team_assignment = [
            -1
        ] * num_total_players_to_assign  # Should match len(active_players)

        # 1. Group available player indices by their position (indices relative to active_players)
        available_player_indices_by_pos = {
            pos: [] for pos in self.position_requirements.keys()
        }
        unrecognized_positions_found = False
        for i, player_data in enumerate(
            active_players
        ):  # Iterate through the (potentially sampled) active_players
            pos = player_data.get("Position")
            if pos in available_player_indices_by_pos:
                available_player_indices_by_pos[pos].append(
                    i
                )  # Store index from active_players
            else:
                ...
                logger.warning(
                    f"Player {i} (Name: {player_data.get('Name', 'N/A')}) has unrecognized position '{pos}'. "
        ...
        if unrecognized_positions_found:
            logger.warning(
                "Some players have positions not in defined requirements and won't be assigned by this method if not handled."
            )
        ...
        for team_id_current in range(self.num_teams):
            ...
            for position_needed, count_needed in self.position_requirements.items():
                for _ in range(
                    count_needed
                ):  # For each slot of this position on this team
                    if available_player_indices_by_pos.get(
                        position_needed
                    ):  # If list exists and is not empty
                        try:
                            # Pop a player index from the available list for this position
                            player_idx_to_assign = available_player_indices_by_pos[
                                position_needed
                            ].pop()
                            ...
                            if player_idx_to_assign in assigned_player_indices:
                                logger.error(
                                    f"Logic error: Player index {player_idx_to_assign} was about to be assigned twice."
                                )
                                # This indicates a problem, potentially try to find another player
                                continue  # Or handle more robustly

                            player_to_team_assignment[player_idx_to_assign] = (
                                team_id_current
                            )
        ...
        if -1 in player_to_team_assignment:
            unassigned_indices = [
                i
                for i, team_id in enumerate(player_to_team_assignment)
                if team_id == -1
            ]
    ...
    def _validate_team_player_counts(self, teams_player_indices: list[list[int]]) -> bool:
        """Checks if each team has the correct number of players."""
        for team_id_check, current_team_player_indices_list in enumerate(
            teams_player_indices
        ):
            if len(current_team_player_indices_list) != self.team_size:
                logger.debug(
                    f"_validate_team_player_counts: False (Team {team_id_check} size {len(current_team_player_indices_list)} != expected {self.team_size})."
                )
                return False
        return True

    def _validate_team_positions_and_budgets(self, teams_player_indices: list[list[int]]) -> bool:
        """Checks position distribution and budget for each team."""
        for team_id_check, current_team_player_indices_list in enumerate(
            teams_player_indices
        ):
            current_team_actual_positions = {
                pos: 0 for pos in self.position_requirements.keys()
            }
            ...
            for player_idx_in_team in current_team_player_indices_list:
                if not (0 <= player_idx_in_team < len(self.players)):
                    logger.debug(
                        f"_validate_team_positions_and_budgets: False (Player index {player_idx_in_team} in team {team_id_check} is out of bounds for self.players)."
                    )
                    return False
                ...
                if player_actual_pos not in current_team_actual_positions:
                    logger.debug(
                        f"_validate_team_positions_and_budgets: False (Team {team_id_check} has player {player_idx_in_team} with unrecognized position '{player_actual_pos}')."
                    )
                    return False
                ...
                try:
                    salary = float(
                        player_data_dict.get(
                            "Salary", player_data_dict.get("Salary (€M)", 0.0)
                        )
                    )
                except (ValueError, TypeError):
                    logger.warning(
                        f"Could not parse salary for player {player_idx_in_team}. Using 0.0."
                    )
                current_team_total_salary += salary

            if current_team_actual_positions != self.position_requirements:
                logger.debug(
                    f"_validate_team_positions_and_budgets: False (Team {team_id_check} positions {current_team_actual_positions} != expected {self.position_requirements})."
                )
                return False

            if current_team_total_salary > self.max_budget:
                logger.debug(
                    f"_validate_team_positions_and_budgets: False (Team {team_id_check} salary {current_team_total_salary} > budget {self.max_budget})."
                )
                return False
        return True
    ...
    def is_valid(self) -> bool:
        ...
        if len(self.repr) != expected_repr_length:
            logger.debug(
                f"is_valid: False (repr length {len(self.repr)} != expected {expected_repr_length})."
            )
            return False
        ...
        if len(self.players) < expected_repr_length:
            logger.debug(
                f"is_valid: False (not enough players in self.players ({len(self.players)}) for repr length {expected_repr_length})."
            )
            return False
        ...
        try:
            for player_idx, team_id in enumerate(self.repr):
                if not (0 <= team_id < self.num_teams):
                    logger.debug(
                        f"is_valid: False (invalid team_id {team_id} for player_idx {player_idx})."
                    )
                    return False
        ...
        if not self._validate_team_player_counts(teams_player_indices):
            return False

        if not self._validate_team_positions_and_budgets(teams_player_indices):
            return False
    ...
    def fitness(self) -> float:
        ...
        for player_idx, team_id in enumerate(self.repr):
            ...
            if 0 <= player_idx < len(self.players):
                ...
                try:
                    team_skills_values_list[team_id].append(
                        self.players[player_idx]["Skill"]
                    )
                except KeyError:
                    logger.error(
                        f"Fitness calculation error: Player index {player_idx} (Name: {self.players[player_idx].get('Name', 'N/A')}) is missing 'Skill' attribute."
                    )
                    self._fitness_cache = float(
                        "inf"
                    )  # Mark as invalid due to data error
                    return self._fitness_cache
            else:
                # This case should ideally be caught by is_valid if repr length matches expected
                logger.error(
                    f"Fitness calculation error: Player index {player_idx} out of bounds. Solution likely invalid."
                )
        ...
        for skills_in_one_team in team_skills_values_list:
            if not skills_in_one_team:
                ...
                logger.warning(
                    "Fitness calculation: An empty team's skill list encountered, though solution was valid. This is unexpected."
                )
    ...
    def get_team_stats(self) -> list[dict]:
        ...
        """
        teams_of_player_dicts = (
            self.get_teams()
        )  # Get list of teams, where each team is list of player dicts
        list_of_team_stats = []
        ...
        for i, single_team_player_list in enumerate(teams_of_player_dicts):
            current_team_stats_dict = {
                ...
                "positions": {
                    pos: 0 for pos in self.position_requirements.keys()
                },  # Initialize with all required positions
                "players_names": [
                    p.get("Name", f"Player_{idx}")
                    for idx, p in enumerate(single_team_player_list)
            ...
            current_team_stats_dict["avg_skill"] = np.mean(
                [p["Skill"] for p in single_team_player_list]
            )

            current_team_total_salary = 0.0
            for p_dict in single_team_player_list:
                try:
                    current_team_total_salary += float(
                        p_dict.get("Salary", p_dict.get("Salary (€M)", 0.0))
                    )
                except (ValueError, TypeError):
                    logger.warning(
                        f"Non-numeric salary for player {p_dict.get('Name', 'Unknown')} in team {i}. Treating as 0."
                    )
            ...
            for p_dict in single_team_player_list:
                ...
                if (
                    player_actual_pos in current_team_stats_dict["positions"]
                ):  # Only count recognized positions
...
class LeagueHillClimbingSolution(LeagueSolution):
    ...
    def get_neighbors(
        self, max_neighbors_total=None
    ):  # Optional: limit total neighbors
        """
        ...
        logger.debug(
            f"Generated {len(same_pos_swap_neighbors)} potential neighbors via same-position swaps."
        )
        ...
        logger.debug(
            f"Generated {len(general_fix_swap_neighbors)} potential neighbors via general inter-team swaps."
        )
        ...
        seen_repr_tuples = (
            set()
        )  # Use a set of tuples (hashable) of repr for uniqueness
        ...
        logger.debug(
            f"Total unique and valid neighbors generated: {len(valid_and_unique_neighbors)}"
        )

        # If a limit is set, sample randomly from the valid unique neighbors
        if (
            max_neighbors_total is not None
            and len(valid_and_unique_neighbors) > max_neighbors_total
        ):
            logger.debug(
                f"Sampling {max_neighbors_total} from {len(valid_and_unique_neighbors)} valid unique neighbors."
            )
    ...
    def _generate_same_position_swaps(self) -> list:
        ...
        player_indices_by_pos = (
            {}
        )  # Map: position_str -> list_of_player_indices_with_that_pos
        for idx, player_data in enumerate(
            self.players
        ):  # Assumes self.players has all players being assigned
            pos = player_data["Position"]
            if pos not in player_indices_by_pos:
                player_indices_by_pos[pos] = []
        ...
        for _, indices_in_pos_category in player_indices_by_pos.items():
            ...
            if num_players_in_cat < 2:
                continue  # Need at least two to swap
            ...
            for i in range(num_players_in_cat):
                for j in range(i + 1, num_players_in_cat):
                    ...
                    if (
                        self.repr[player1_original_idx]
                        != self.repr[player2_original_idx]
                    ):
                        new_repr_list = list(self.repr)  # Create a mutable copy
                        # Swap team assignments for these two players
                        (
                            new_repr_list[player1_original_idx],
                            new_repr_list[player2_original_idx],
                        ) = (
                            new_repr_list[player2_original_idx],
                            new_repr_list[player1_original_idx],
                        )
                        ...
                        neighbor_sol = (
                            self.__class__(  # Instantiates LeagueHillClimbingSolution
                                repr=new_repr_list,
    ...
    def _generate_general_inter_team_swaps(self) -> list:
        ...
        for team_a_id in range(self.num_teams):
            for team_b_id in range(team_a_id + 1, self.num_teams):
                ...
                team_a_player_indices_in_repr = [
                    idx for idx, t_id in enumerate(self.repr) if t_id == team_a_id
                ]
                team_b_player_indices_in_repr = [
                    idx for idx, t_id in enumerate(self.repr) if t_id == team_b_id
                ]

                if (
                    not team_a_player_indices_in_repr
                    or not team_b_player_indices_in_repr
                ):
                ...
                for p1_main_idx in team_a_player_indices_in_repr:
                    for p2_main_idx in team_b_player_indices_in_repr:
                        ...
                        new_repr_list[p1_main_idx], new_repr_list[p2_main_idx] = (
                            new_repr_list[p2_main_idx],
                            new_repr_list[p1_main_idx],
                        )
...
# Import necessary mutation operators
from mutation_operators import mutate_swap_constrained, mutate_swap
...
class LeagueSASolution(LeagueSolution):
    ...
    def get_random_neighbor(self):
        """
        Generate a random valid neighboring solution by applying a randomly chosen mutation operator.
        """
        if not self.players or len(self.repr) < 2:
            logger.debug(
                "SA get_random_neighbor: No players or not enough players for a swap."
            )
            return deepcopy(self) # Return a deepcopy as per original behavior for this case

        max_attempts_to_find_valid = 50
        for _ in range(max_attempts_to_find_valid):
            temp_sol = deepcopy(self) # Create a copy to mutate
            
            mutated_sol_object = None
            if random.random() < 0.5: # 50% chance for constrained swap
                mutated_sol_object = mutate_swap_constrained(temp_sol)
                logger.debug("SA get_random_neighbor: Attempting mutate_swap_constrained.")
            else: # 50% chance for general swap
                mutated_sol_object = mutate_swap(temp_sol)
                logger.debug("SA get_random_neighbor: Attempting mutate_swap.")
            
            # The mutation operators return new instances.
            # Check if the new instance is valid.
            if mutated_sol_object is not None and mutated_sol_object.is_valid():
                logger.debug("SA get_random_neighbor: Found valid random neighbor.")
                return mutated_sol_object
            else:
                logger.debug("SA get_random_neighbor: Generated invalid neighbor or mutation failed, retrying.")

        # If max_attempts reached and no valid neighbor found, return a new deepcopy of self
        logger.debug(
            "SA get_random_neighbor: Max attempts reached, returning copy of current solution."
        )
