...
from solution import InsufficientPlayersForPositionError, LeagueSolution
...
def basic_problem_setup():
    ...
    players_data = [
        ...
        {"Name": "Player6", "Position": "DEF", "Skill": 77, "Salary": 130},
        {
            ...
            "Skill": 82,
            "Salary": 160,
        },  # Extra MID for 'too_many'
        {
            "Name": "Player8",
            "Position": "DEF",
            "Skill": 78,
            "Salary": 140,
        },  # Extra DEF for 'too_many'
    ]
...
def test_random_initial_too_few_players(basic_problem_setup):
    ...
    players = basic_problem_setup["players_data"][
        : pd["num_teams"] * pd["team_size"] - 1
    ]
...
def test_random_initial_too_many_players(basic_problem_setup):
    ...
    assert (
        len(sol.players) == expected_player_count
    ), "self.players was not down-sampled"
    assert sol.repr is not None
    assert len(sol.repr) == expected_player_count
    assert all(
        team_id != -1 for team_id in sol.repr
    ), "Not all players (from sampled list) were assigned"
...
def test_random_initial_insufficient_players_for_position(basic_problem_setup):
    ...
    with pytest.raises(
        ...
        match="Insufficient players of position 'GK'",
    ):
        LeagueSolution(
...
def test_random_initial_valid_assignments(basic_problem_setup):
    ...
    for team_id, team_players in enumerate(teams_check):
        assert (
            len(team_players) == pd["team_size"]
        ), f"Team {team_id} has incorrect size"
        ...
        assert (
            current_team_positions == pd["position_requirements"]
...
def test_fitness_caching_deepcopy(basic_problem_setup):
    ...
    assert sol_copy.fitness() == pytest.approx(
        original_fitness
    ), "Deepcopy did not use its own cache or cache was wrong"
    assert sol_orig.fitness() == pytest.approx(
        dummy_val_for_orig_cache
    ), "Original's cache was not changed as expected"
