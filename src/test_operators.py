...
def operator_test_setup():
    ...
    players_data = [
        ...
        {"Name": "P2", "Position": "MID", "Skill": 80, "Salary": 150},
        {
            ...
            "Skill": 72,
            "Salary": 110,
        },  # Enough GKs for 2 teams
        {
            "Name": "P4",
            "Position": "DEF",
            "Skill": 77,
            "Salary": 130,
        },  # Enough DEFs for 2 teams
        {
            "Name": "P5",
            "Position": "MID",
            "Skill": 82,
            "Salary": 160,
        },  # Enough MIDs for 2 teams
    ...
    if sum(problem_definition["position_requirements"].values()) != team_size:
        raise ValueError(
            "Test setup error: Sum of position_requirements must equal team_size."
        )
    ...
    if len(players_data) < total_players_needed_for_one_solution:
        raise ValueError(
            f"Test setup error: players_data has {len(players_data)} players, "
    ...
    def create_valid_solution(specific_players=None, specific_repr=None):
        ...
        current_players_list = (
            deepcopy(specific_players) if specific_players else deepcopy(players_data)
        )
...
def test_mutate_swap_constrained(operator_test_setup):
    ...
    if not original_solution.is_valid():
        ...
        pytest.skip(
            "Initial random solution for test_mutate_swap_constrained was not valid."
        )
    ...
    if mutated_solution.repr != original_solution.repr:
        diff_indices = [
            ...
            for i, (o, m) in enumerate(
                zip(original_solution.repr, mutated_solution.repr)
            )
            if o != m
        ]
        ...
        assert (
            original_p1_pos == original_p2_pos
        ), "Constrained swap should swap players of same position"
...
def test_mutate_shuffle_within_team_constrained_corrected(operator_test_setup):
    ...
    if not original_solution.is_valid():
        pytest.skip(
            "Initial solution for test_mutate_shuffle_within_team_constrained was not valid."
        )
    ...
    if mutated_solution.repr != original_solution.repr:
        ...
        swapped_indices = [
            i
            for i, (o_team, m_team) in enumerate(
                zip(original_solution.repr, mutated_solution.repr)
            )
            if o_team != m_team
        ]
        assert (
            len(swapped_indices) == 2
        ), "Exactly two players should have their teams changed"
        ...
        assert (
            original_p1_team != original_p2_team
        ), "Swapped players must have been on different teams"
...
def test_mutate_targeted_player_exchange(operator_test_setup):
    create_sol = operator_test_setup["create_valid_solution"]
    players = operator_test_setup["players_data"][
        : operator_test_setup["total_players_needed"]
    ]
    ...
    original_solution = create_sol(
        specific_repr=[0, 0, 0, 1, 1, 1]
    )  # P0,P1,P2 on T0; P3,P4,P5 on T1
    if not original_solution.is_valid():
        pytest.skip(
            "Initial solution for mutate_targeted_player_exchange was not valid."
        )
    ...
    if mutated_solution.repr != original_solution.repr:
        ...
        assert (
            original_solution.players[p1_idx]["Position"]
            == original_solution.players[p2_idx]["Position"]
        )
...
def test_crossover_one_point_prefer_valid(operator_test_setup):
    ...
    parent2 = create_sol(
        specific_repr=[1, 1, 1, 0, 0, 0]
    )  # T0: P3,P4,P5 ; T1: P0,P1,P2
    ...
    assert (
        child.is_valid()
    ), "Child from prefer_valid crossover should be valid if parents are"
...
def test_selection_tournament_variable_k(operator_test_setup):
    ...
    base_players = operator_test_setup["players_data"][
        : operator_test_setup["total_players_needed"]
    ]
    ...
    for i in range(population_size):
        ...
        # T0: P0,P1,P2 ; T1: P3,P4,P5
        sol = create_sol(
            specific_players=current_players, specific_repr=[0, 0, 0, 1, 1, 1]
        )
    ...
    if len(population) < 2:  # Need at least a few for meaningful selection test
        pytest.skip(
            "Could not create a sufficiently large valid population for selection test."
        )
    ...
    selected_k_large = selection_tournament_variable_k(
        population, k=len(population) + 5
    )
...
def test_crossover_two_point_valid_parents(operator_test_setup):
    ...
    assert (
        parent1.is_valid() and parent2.is_valid()
    ), "Parents must be valid for this test"
    ...
    assert (
        child.is_valid()
    ), "Child from controlled two-point crossover should be valid if parents are and repr is constructed correctly"
...
def test_crossover_two_point_short_representation(operator_test_setup):
    ...
    short_problem_def["position_requirements"] = {
        "GK": 1
    }  # Assuming P0, P1 are GKs or compatible

    # Modify players to fit simple position req
    short_players_data[0]["Position"] = "GK"
    short_players_data[1]["Position"] = "GK"

    parent1 = LeagueSolution(
        repr=[0],
        players=[short_players_data[0]],
        num_teams=1,
        team_size=1,
        max_budget=100,
        position_requirements={"GK": 1},
    )
    parent2 = LeagueSolution(
        repr=[0],
        players=[short_players_data[1]],
        num_teams=1,
        team_size=1,
        max_budget=100,
        position_requirements={"GK": 1},
    ...
    p_fitter["Skill"] = (
        100  # Higher skill = lower (better) team skill_std_dev if teams are uniform
    )
    p_less_fit["Skill"] = 50
    ...
    parent1_sol = LeagueSolution(
        [0, 1],
        num_teams=2,
        team_size=1,
        max_budget=200,
        players=players_for_len2,
        position_requirements={"GK": 1},
    )
    parent2_sol = LeagueSolution(
        [1, 0],
        num_teams=2,
        team_size=1,
        max_budget=200,
...
def test_crossover_two_point_prefer_valid_produces_valid_child(operator_test_setup):
    ...
    mock_child_solution = create_sol(
        specific_repr=[0, 0, 0, 0, 1, 1]
    )  # Expected from a specific crossover
    mock_child_solution.is_valid = MagicMock(return_value=True)

    with patch(
        "tests.test_operators.crossover_two_point", return_value=mock_child_solution
    ) as mock_crossover_op:
        ...
        assert (
            child is mock_child_solution
        )  # Should be the one returned by the mocked op


def test_crossover_two_point_prefer_valid_produces_invalid_then_valid(
    operator_test_setup,
):
    create_sol = operator_test_setup["create_valid_solution"]
    parent1 = create_sol(specific_repr=[0, 0, 0, 1, 1, 1])
    ...
    with patch("tests.test_operators.crossover_two_point") as mock_crossover_op:
        mock_crossover_op.side_effect = [
            invalid_child,
            valid_child,
            invalid_child,
        ]  # Third one shouldn't be called
...
def test_crossover_two_point_prefer_valid_always_invalid_returns_fitter_parent(
    operator_test_setup,
):
    create_sol = operator_test_setup["create_valid_solution"]
    ...
    with patch(
        "tests.test_operators.crossover_two_point", return_value=invalid_child
    ) as mock_crossover_op:
        returned_child = crossover_two_point_prefer_valid(
            parent1, parent2, max_attempts=3
        )
...
def test_crossover_two_point_prefer_valid_parents_differing_fitness(
    operator_test_setup,
):
    create_sol = operator_test_setup["create_valid_solution"]
    parent1 = create_sol(specific_repr=[0, 0, 0, 1, 1, 1])
    ...
    invalid_child_mock = (
        create_sol()
    )  # Create a valid solution instance just for mocking
    invalid_child_mock.is_valid = MagicMock(return_value=False)

    with patch(
        "tests.test_operators.crossover_two_point", return_value=invalid_child_mock
    ) as mock_basic_crossover:
        result_child1_fitter = crossover_two_point_prefer_valid(
            parent1, parent2, max_attempts=2
        )
    ...
    mock_basic_crossover.reset_mock(
        return_value=invalid_child_mock, side_effect=None
    )  # Reset side_effect too
    ...
    with patch(
        "tests.test_operators.crossover_two_point", return_value=invalid_child_mock
    ) as mock_basic_crossover_2:
        result_child2_fitter = crossover_two_point_prefer_valid(
            parent1, parent2, max_attempts=2
        )
...
def test_genetic_algorithm_island_model_integration(operator_test_setup):
    ...
    test_island_model_ga_params = {
        ...
        "ga_params_per_island": {
            ...
            "crossover_operator": operator_test_setup[
                "crossover_one_point_prefer_valid"
            ],
            "crossover_rate": 0.8,
            "mutation_operator": operator_test_setup[
                "mutate_swap_constrained"
            ],  # Using constrained version
            "mutation_rate": 0.2,
            "elitism": True,
            "elitism_size": 1,
            "verbose": False,
    ...
    ), "Best solution should be an instance of LeagueSolution."
    ...
    if not is_valid_check:
        ...
        pytest.fail(
            f"Best solution from Island GA is not valid. Violations: {violations}"
        )

    assert isinstance(best_fitness, float), "Best fitness should be a float."
    assert best_fitness != float(
        "inf"
    ...
    assert (
        len(history) == test_island_model_ga_params["max_generations_total"] + 1
    ...
    ), "Best fitness should match the last entry in history."
